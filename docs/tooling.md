...
The root project uses `module: Preserve` with its implied bundler resolution for Vite browser code and GPU tests. It includes root config files for Oxlint project discovery; the separate Node check also verifies those files under `module: NodeNext` and Node libraries. `tsconfig.node.json` describes configuration, CPU tests, references, and CPU benchmarks. The render worker has its own WebWorker-only library scope. Small forwarding configs let Oxlint find the correct project for each test environment; they do not duplicate compiler policy.
...
| HTML popovers, dialog commands, exclusive `details`                               | Native dismissal, focus, and settings groups                                                             |
| CSS nesting/layers, relative OKLCH, subgrid, anchors, `@starting-style`, `:has()` | Shared styles, color tokens, aligned controls, positioning, entry transitions, and empty feedback layout |
...
Optical timing requires `timestamp-query`. A missing timestamp capability is explicitly skipped without inventing a CPU substitute. Readbacks, assertions, hashes, and metadata remain outside the measured interval. Queue writes, submission latency, and presentation are excluded from optical timestamps. Stage names and scope are recorded with each result.

Results under `test-results/bench/` include exact image readbacks, source/data fingerprints, sample policy, adapter/browser identity, queue counts, unresolved coverage, and discarded timestamps. Compare equal inputs and image quality before claiming a speedup. Pause other renderer previews, serialize GPU workloads, and record any unverified concurrency. [Validation](validation.md#performance) defines interpretation and acceptance.
