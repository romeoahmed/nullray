...
import { createAccumulation } from "../../src/gpu/accumulation.ts";
import { createCoverageReader } from "../../src/gpu/coverage.ts";
...
describe("Boundary quadrature", () => {
      ...
      if (previous) {
        ...
        for (const record of current.interiorErrors) {
          ...
          for (let channel = 0; channel < 3; channel++) {
            ...
            error += Math.abs(
              (record.residual[channel] ?? NaN) - (before.residual[channel] ?? NaN),
            );
          }
        }
  ...
  test("exhausted boundary queues explicitly retain unresolved pixels", async ({ device }) => {
    using owned = new DisposableStack();

    device.pushErrorScope("validation");
    const texture = (format: GPUTextureFormat, usage: GPUTextureUsageFlags) =>
      owned.adopt(device.createTexture({ size: [4, 4], format, usage }), (value) =>
        value.destroy(),
      );
