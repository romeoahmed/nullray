...
export interface BeamQueue {
  ...
  /** Encode candidate selection and one differentiated ray per retained candidate. Clear statistics first. */
...
export function createBeamQueue(
  ...
  return {
    ...
    encode(encoder) {
      ...
      repairPass.dispatchWorkgroups(Math.ceil(capacity / 32));
      repairPass.end();
    },
  };
}
