...
fn d_polar_motion(status: ptr<function, bool>, a: f32, e: vec3f, l: vec3f, c: vec3f,
  ...
  let coefficient = d_div(status, d_sqrt(status, d_add(status, d_div(status, c, z), ae2)), frequency);
  return DifferentialPolar(phase, frequency, m, z, d_mul(status, root, root), root, coefficient, true, d_prepare_jacobi(status, m));
}
...
fn d_polar_base(status: ptr<function, bool>, motion: DifferentialPolar, events: EquatorEvents, l: f32, end: vec3f) -> vec3f {
  ...
  return d_scale(status, d_mul(status, motion.coefficient, vec3f(unwrapped, gradient.yz)), -1.0);
}
...
fn d_sky_rate(status: ptr<function, bool>, radial: DifferentialQuartic, polar: DifferentialQuartic,
  ...
  if (motion.enabled) {
    ...
    let denominator = d_mul(status, motion.amplitude2, d_add(status, d_constant(status, 1.0), j.dn));
    // At L=0 the residual's integral derivative has this continuous almost-everywhere limit.
    // A single pole time has zero integration measure; no 0/0 arithmetic is evaluated there.
    polar_rate = d_div(status, d_mul(status, l, motion.m), denominator);
    if (l.x != 0.0) {
      let sin2 = d_add(status, motion.complement, d_mul(status, motion.amplitude2, d_mul(status, j.sn, j.sn)));
      polar_rate = d_div(status, d_mul(status, d_mul(status, l, motion.m), d_mul(status, j.sn, j.sn)),
        d_mul(status, d_add(status, d_constant(status, 1.0), j.dn), sin2));
    }
...
fn d_sky_transport(status: ptr<function, bool>, radial: DifferentialQuartic, polar: DifferentialQuartic,
    a: f32, q: f32, e: vec3f, l: vec3f, c: vec3f, end: vec3f, mu: vec3f, launch: vec3f, motion: DifferentialPolar) -> vec3f {
  // The regular residual and all screen derivatives vanish identically at
  // fixed a=0. d_polar_base already supplies the complete angular transport.
  if (a == 0.0 && motion.enabled) { return vec3f(0.0); }
  let phase = end.x * sqrt(l.x * l.x + 2.0 * abs(c.x) + 2.0 * a * a * e.x * e.x);
  var count = 32u;
  for (var i = 0u; i < 6u && f32(count) < 8.0 * phase; i++) { count *= 2u; }
  if (count > 256u) { return d_fail(status); }
  // Four interleaved sums shorten serial rounding chains. Their balanced
  // reduction changes no quadrature nodes, convergence thresholds, or budget.
  var sums: array<vec3f, 4>;
  var areas: array<f32, 4>;
  for (var i = 1u; i < count; i++) {
    let rate = d_warped_sky_rate(status, radial, polar, a, q, e, l, end, f32(i) / f32(count), motion);
    sums[i % 4u] = d_add(status, sums[i % 4u], rate);
    areas[i % 4u] += d_sky_area_rate(status, mu, rate);
  }
  let sum = d_add(status, d_add(status, sums[0], sums[1]), d_add(status, sums[2], sums[3]));
  var trapezoid = d_scale(status, sum, 1.0 / f32(count));
  var previous = vec3f(0.0);
  var area_trapezoid = ((areas[0] + areas[1]) + (areas[2] + areas[3])) / f32(count);
  var previous_area = 0.0;
  for (var level = 0u; level < 6u && count < 2048u; level++) {
    var midpoints: array<vec3f, 4>;
    var midpoint_areas: array<f32, 4>;
    for (var i = 0u; i < count; i++) {
      let rate = d_warped_sky_rate(status, radial, polar, a, q, e, l, end, (f32(i) + 0.5) / f32(count), motion);
      midpoints[i % 4u] = d_add(status, midpoints[i % 4u], rate);
      midpoint_areas[i % 4u] += d_sky_area_rate(status, mu, rate);
    }
    if (!(*status)) { return vec3f(0.0); }
    let midpoint_sum = d_add(status, d_add(status, midpoints[0], midpoints[1]), d_add(status, midpoints[2], midpoints[3]));
    let next = d_add(status, d_scale(status, trapezoid, 0.5), d_scale(status, midpoint_sum, 1.0 / (2.0 * f32(count))));
    let estimate = d_scale(status, d_sub(status, d_scale(status, next, 4.0), trapezoid), 1.0 / 3.0);
    let area = mu.y * (estimate.z + launch.z) - mu.z * (estimate.y + launch.y);
    let next_area = 0.5 * area_trapezoid + ((midpoint_areas[0] + midpoint_areas[1])
      + (midpoint_areas[2] + midpoint_areas[3])) / (2.0 * f32(count));
    let estimated_area = (4.0 * next_area - area_trapezoid) / 3.0;
    let area_change = estimated_area - previous_area;
    if (level > 0u && abs(area) > 1e-20
      && abs(area_change) <= 1e-4 * abs(area)
      && all(abs(estimate - previous) <= 1e-5 * (vec3f(1.0) + abs(estimate)))) {
      return d_add(status, estimate, d_scale(status, d_sub(status, estimate, previous), 1.0 / 15.0));
    }
    previous = estimate;
    previous_area = estimated_area;
    area_trapezoid = next_area;
    trapezoid = next;
    count *= 2u;
  }
 ...
 */
fn differential_sky(frame: Frame, pixel: vec2f, expected_branch: f32) -> DifferentialSky {
  ...
  if (theta == 1.5707963268 && source[1].x == 0.0) { return differential_sky_failure(); }
  ...
  let polar = d_prepare_quartic(status, array<vec3f, 5>(c, d_constant(status, 0.0), quadratic, d_constant(status, 0.0), d_scale(status, e2, -a * a)),
    d_constant(status, cosine), d_scale(status, p_theta, sine));
  if (!(*status) || e.x <= 0.0) { return differential_sky_failure(); }
  ...
  if (destination != 1u) { return differential_sky_failure(); }
  let arrival = quartic_arrival(primal, 0.0, -e.x);
  if (arrival.status != 1u) { return differential_sky_failure(); }
  let time = (arrival.lower + arrival.upper) / 2.0;
  if ((arrival.upper - arrival.lower) * sqrt(abs(c.x) + l.x * l.x + a * a * e.x * e.x) > 4e-5) { return differential_sky_failure(); }
  let events = equator_events(a, e.x, l.x, c.x, theta, p_theta.x);
  if (events.status == 2u) { return differential_sky_failure(); }
  ...
  if (!(*status)) { return differential_sky_failure(); }
  if (l.x == 0.0 && !motion.enabled) {
    if (any(l.yz != vec2f(0.0)) || events.status != 1u) { return differential_sky_failure(); }
  ...
  var branch = 1.0;
  let primal_plan = prepare_jacobi(primal.elliptic.m);
  for (var index = 0u; index < 64u && events.status == 1u && next <= arrival.upper; index++) {
    if (next >= arrival.lower) { return differential_sky_failure(); }
    let hit = evaluate_quartic_prepared(primal, next, primal_plan);
    if (!hit.valid || (hit.value >= 1.0 / frame.space.w && hit.value <= 1.0 / frame.space.z)) { return differential_sky_failure(); }
    branch += 1.0;
    next += events.spacing;
  }
  if ((events.status == 1u && next <= arrival.upper) || branch != expected_branch) { return differential_sky_failure(); }
  ...
  if (!(*status) || abs(mu.x) >= 1.0) { return differential_sky_failure(); }
  if (motion.enabled) { launch_phi = d_add(status, launch_phi, d_polar_base(status, motion, events, l.x, end)); }
  if (!(*status)) { return differential_sky_failure(); }
  let phi = d_add(status, d_sky_transport(status, radial, polar, a, q, e, l, c, end, mu, launch_phi, motion), launch_phi);
  return DifferentialSky(mu, phi, e.x, (*status));
}
