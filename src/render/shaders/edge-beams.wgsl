...
fn repair_edge_beams(@builtin(global_invocation_id) id: vec3u) {
  ...
  let center = vec2f(boundary_pixels[boundary]) + (vec2f(pixel % 4u) + 0.5) / 4.0 - 0.5;
  repair_differential_beam(id.x, center);
}
