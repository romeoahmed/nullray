import source from "./shaders/bloom.wgsl?raw";
import { compileShader } from "./device.ts";

/** Own a normalized six-scale radiance filter. The caller decides when its source changes. */
export async function createBloom(device: GPUDevice) {
  ...
  let width = 0;
  let height = 0;
  let levels: { readonly down: GPUTexture; readonly up: GPUTexture }[] = [];
  ...
  function preparePass(input: GPUTexture, output: GPUTexture, detail?: GPUTexture) {
    ...
    if (detail) {
      entries.push({ binding: 2, resource: detail.createView() });
    }
    return {
      pipeline: selected,
    ...
    encode(
      ...
      if (width !== radiance.width || height !== radiance.height) {
        ...
        for (let level = 0; level < 6; level++) {
          ...
          levels.push({ down: texture(w, h), up: texture(w, h) });
          if (w === 1 && h === 1) {
            break;
          }
        }
      ...
      if (input !== radiance) {
        ...
        for (let index = levels.length - 2; index >= 0; index--) {
          ...
          if (level) {
            passes.push(preparePass(result, level.up, level.down));
            result = level.up;
          }
        }
        input = radiance;
