...
@group(0) @binding(8) var<uniform> observer_frame: Observer64;
...
fn disk_variation(a: vec4f, b: vec4f, delay: f32) -> bool {
  ...
  // Each unit-amplitude mode changes by at most two. Keep that bound per mode
  // before summing; a fine mode cannot consume more than twice its flux weight.
  let first = min(vec4f(2.0), vec4f(2.0, 5.0, 11.0, 23.0) * phase
    + 3.0 * vec4f(2.0, 4.0, 8.0, 16.0) * radial);
  let second = min(vec4f(2.0), vec4f(3.0, 7.0, 15.0, 31.0) * phase
    + 3.0 * vec4f(3.0, 6.0, 12.0, 24.0) * radial);
  return dot(vec4f(0.16, 0.13, 0.1, 0.06), first)
    + dot(vec4f(0.19, 0.16, 0.12, 0.08), second) + abs(delay) / 8.0 > 1.0
    || abs(log(b.z / a.z)) > 0.03;
}
...
fn sample_edges(@builtin(global_invocation_id) id: vec3u) {
  ...
  let endpoint = screen_ray_refined(frame, vec2f(edge_pixels[index]) + offset, observer_frame);
