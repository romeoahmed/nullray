...
/**
 * Unit-integral screen-space tent applied to a normalized affine source mapping.
 ...
 */
fn point_stars(direction: vec3f, gradient_x: vec3f, gradient_y: vec3f, energy: f32, footprint: StellarFootprint) -> vec4f {
    ...
    if (node.lower.w >= 0.0) {
      ...
      if (alignment > 0.0) {
        ...
        if (all(abs(numerator) < vec2f(abs(denominator) * alignment))) {
          let position = (numerator / denominator) / alignment;
          if (stellar_filtered_image(position, footprint)) { index += 1u; continue; }
          let weight = 1.0 - abs(position);
          if (node.upper.x > 0.0) {
            ...
            // n(X,Y) = normalize(n0 + dx*X + dy*Y) has
            // J(image) = J(center) * dot(n(image), n0)^3.
            // Balance the two scale divisions before the remaining alignment
            // division; forming either scale² or alignment³ can overflow/underflow.
            let projected_scale = scale * alignment;
            if (!(projected_scale > 0.0) || !(sqrt(abs(area)) * sqrt(alignment) > 1e-10 / projected_scale)) {
              return vec4f(0.0);
            }
            let contribution = node.upper.x * spectrum.rgb * weight.x * weight.y;
            radiance += ((contribution / projected_scale) / (abs(denominator) * alignment)) / alignment;
          }
        }
      }
    }
