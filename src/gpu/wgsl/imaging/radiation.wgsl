...
/** Four periodic appearance modes with independently localized radial phases. */
fn disk_modes(log_radius: f32, phase: f32, epoch: f32, modes: vec4u, density: vec4f) -> vec4f {
  let coordinate = log_radius * density;
  ...
  return mix(sin(angle + disk_phase(bitcast<vec4u>(cell), modes, epoch)),
    sin(angle + disk_phase(bitcast<vec4u>(cell + 1), modes, epoch)), blend);
}

/** Zero-mean bounded appearance at r in M; the eight weights sum to one. */
fn disk_structure(r: f32, inner: f32, phase: f32, epoch: f32) -> f32 {
  let log_radius = log(r / inner);
  // Interleaved scales avoid concentrating most contrast in two broad azimuthal bands.
  let first = disk_modes(log_radius, phase, epoch, vec4u(2u, 5u, 11u, 23u), vec4f(2.0, 4.0, 8.0, 16.0));
  let second = disk_modes(log_radius, phase, epoch, vec4u(3u, 7u, 15u, 31u), vec4f(3.0, 6.0, 12.0, 24.0));
  return dot(vec4f(0.16, 0.13, 0.1, 0.06), first) + dot(vec4f(0.19, 0.16, 0.12, 0.08), second);
}
