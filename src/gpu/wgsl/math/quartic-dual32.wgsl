 ...
 */
fn d_prepare_jacobi_pair(status: ptr<function, bool>, m: vec3f, complement: vec3f) -> DifferentialJacobiPlan {
  ...
  if (m.x <= 0.0 || complement.x <= 0.0) { return plan; }
  var a = d_constant(status, 1.0);
  var b = d_sqrt(status, complement);
  var multiplier = 1.0;
...
fn d_prepare_jacobi(status: ptr<function,bool>, m: vec3f) -> DifferentialJacobiPlan {
  return d_prepare_jacobi_pair(status,m,d_sub(status,d_constant(status,1.0),m));
}
...
fn d_evaluate_jacobi(status: ptr<function, bool>, plan: DifferentialJacobiPlan, u: vec3f) -> DifferentialJacobi {
  ...
  let dn = d_sqrt(status, d_add(status, plan.complement, d_mul(status, m, d_mul(status, cn, cn))));
...
struct DifferentialElliptic {
  g2: vec3f, g3: vec3f, root: vec3f, scale: vec3f, m: vec3f, complement: vec3f, three_real: bool,
...
fn d_prepare_elliptic(status: ptr<function, bool>, g2: vec3f, g3: vec3f) -> DifferentialElliptic {
  ...
  if (g2.x > 0.0 && discriminant.x >= 0.0) {
    ...
    return DifferentialElliptic(g2, g3, e3, scale, m, d_sub(status,d_constant(status,1.0),m), true, d_prepare_jacobi(status, m));
  }
  ...
  return DifferentialElliptic(g2, g3, e, scale, m, d_sub(status,d_constant(status,1.0),m), false, d_prepare_jacobi(status, m));
}
...
fn d_primal_quartic(path: DifferentialQuartic) -> QuarticPath {
  ...
  return QuarticPath(path.x.x, path.velocity.x, path.f.x, path.first.x, path.second.x, path.third.x, path.fourth.x,
    Elliptic(e.g2.x, e.g3.x, e.root.x, e.scale.x, e.m.x, e.complement.x, e.three_real));
}
...
fn d_evaluate_quartic(status: ptr<function, bool>, path: DifferentialQuartic, t: vec3f) -> vec3f {
  ...
  if (all(path.third == vec3f(0.0)) && all(path.fourth == vec3f(0.0)) && path.second.x != 0.0) {
    var squared = d_scale(status,path.second,0.5);
    if (squared.x < 0.0) { squared = d_scale(status,squared,-1.0); }
    let frequency = d_sqrt(status,squared);
    let phase = d_mul(status,frequency,t);
    let acceleration = d_div(status,path.first,path.second);
    let velocity = d_div(status,path.velocity,frequency);
    if (path.second.x < 0.0) {
      if (abs(phase.x) > 0.1) {
        return d_add(status,d_add(status,d_sub(status,path.x,acceleration),
          d_mul(status,acceleration,d_cos(status,phase))),d_mul(status,velocity,d_sin(status,phase)));
      }
      let half = d_sin(status,d_scale(status,phase,0.5));
      return d_add(status,d_add(status,path.x,d_scale(status,d_mul(status,acceleration,d_mul(status,half,half)),-2.0)),
        d_mul(status,velocity,d_sin(status,phase)));
    }
    if (abs(phase.x) > log(1e30)) { return d_fail(status); }
    let half_phase = d_scale(status,phase,0.5);
    let half = d_compose(status,half_phase,sinh(half_phase.x),cosh(half_phase.x));
    let sine = d_compose(status,phase,sinh(phase.x),cosh(phase.x));
    return d_add(status,d_add(status,path.x,d_scale(status,d_mul(status,acceleration,d_mul(status,half,half)),2.0)),
      d_mul(status,velocity,sine));
  }
  let p = path.elliptic;
  ...
  if (abs(g2t4.x) + abs(g3t6.x) >= 1e-3) {
    ...
    if (!p.three_real) {
      ...
      slope_factor = d_mul(status, j.cn, d_add(status, p.complement, d_mul(status, p.m, d_mul(status, cn2, cn2))));
    }
