...
fn prepare_elliptic(g2: f32, g3: f32) -> Elliptic {
  ...
  if (g2 > 0.0 && discriminant >= 0.0) {
    ...
    let e2 = -e1 - e3;
    let m = clamp((e2 - e3) / (e1 - e3), 0.0, 1.0);
    return Elliptic(g2, g3, e3, e1 - e3, m, 1.0 - m, true);
  }
  ...
  return Elliptic(g2, g3, e, scale, m, 1.0 - m, false);
}
 ...
 */
fn prepare_jacobi_pair(m: f32, complement: f32) -> JacobiPlan {
  var ratios: array<f32, 8>;
  if (m == 0.0 || complement == 0.0) { return JacobiPlan(m, complement, 1.0, 1.0, 6.283185307, ratios, 0u); }
  var a = 1.0;
  var b = sqrt(complement);
  var multiplier = 1.0;
  var count = 0u;
  for (var i = 0u; i < 8u; i++) {
    let next_a = (a + b) / 2.0;
  ...
  return JacobiPlan(m, complement, a, multiplier, 6.283185307 / a, ratios, count);
}

/** Native parameters retain their directly computed complement. */
fn prepare_jacobi(m: f32) -> JacobiPlan {
  return prepare_jacobi_pair(m,1.0-m);
}
...
fn evaluate_jacobi(plan: JacobiPlan, u: f32) -> vec3f {
  ...
  return vec3f(sn, cn, sqrt(plan.complement + m * cn * cn));
}
...
fn evaluate_quartic_kernel(path: QuarticPath, t: f32, plan: JacobiPlan, prepared: bool) -> Evaluation {
  ...
  if (path.third == 0.0 && path.fourth == 0.0 && path.second != 0.0) {
    // Exact quadratic motion avoids a removable elliptic quotient whose
    // subtraction is poorly conditioned near the trigonometric/hyperbolic limits.
    let frequency = sqrt(abs(path.second) * 0.5);
    if (!(frequency > 0.0) || abs(t) > 1e30 / max(1.0,frequency)) { return Evaluation(0.0,false); }
    if (abs(path.second) < 1.0 && abs(path.first) > 1e30 * abs(path.second)) { return Evaluation(0.0,false); }
    if (frequency < 1.0 && abs(path.velocity) > 1e30 * frequency) { return Evaluation(0.0,false); }
    let phase = frequency * t;
    let acceleration = path.first / path.second;
    let velocity = path.velocity / frequency;
    var sine = sin(phase);
    var cosine = cos(phase);
    var increment = -2.0 * sin(phase * 0.5) * sin(phase * 0.5);
    if (path.second > 0.0) {
      if (abs(phase) > log(1e30)) { return Evaluation(0.0,false); }
      sine = sinh(phase);
      cosine = cosh(phase);
      let half = sinh(phase * 0.5);
      increment = 2.0 * half * half;
    }
    if (abs(acceleration) > 1e30 / max(1.0,abs(cosine)) ||
        abs(velocity) > 1e30 / max(1.0,abs(sine)) || abs(path.x) > 1e30) { return Evaluation(0.0,false); }
    if (path.second < 0.0 && abs(phase) > 0.1) {
      return Evaluation((path.x-acceleration) + acceleration*cosine + velocity*sine,true);
    }
    return Evaluation(path.x + acceleration*increment + velocity*sine,true);
  }
  let p = path.elliptic;
  let t2 = t * t;
  let t4 = t2 * t2;
  ...
  if (abs(g2t4) + abs(g3t6) >= 1e-3) {
    ...
    else { j = evaluate_jacobi(prepare_jacobi_pair(p.m,p.complement),root_scale * t); }
    var z = j.x;
    ...
    if (!p.three_real) {
      ...
      slope = j.y * (p.complement + p.m * cn2 * cn2);
    }
    let z2 = z * z;
    let z4 = z2 * z2;
