...
fn refine_arrival(path: QuarticPath, seed: f32, endpoint: f32, velocity: f32) -> Arrival {
  ...
  let plan = prepare_jacobi_pair(path.elliptic.m,path.elliptic.complement);
  ...
  for (var iteration = 0u; iteration < 12u; iteration++) {
    ...
    // Keep refining while the bracket has a representable midpoint. A few
    // time ULPs can still prevent convergence of a high-order stellar image.
...
fn quartic_arrival(path: QuarticPath, endpoint: f32, velocity: f32) -> Arrival {
  ...
  // Sum magnitudes before cancellation in H; its rounded result understates
  // uncertainty for distant observers whose boundary lies near a half-period.
  let combined_scale = abs(path.f) + abs(path.velocity * velocity) + abs(path.first * difference / 2.0);
  let uncertainty = 32.0 * 1.1920928955e-7 * (combined_scale / (2.0 * d2) + abs(path.second / 24.0) + abs(minimum));
  if (abs(wp - minimum) <= uncertainty && p.complement > 0.0) {
    let curvature = 6.0 * minimum * minimum - p.g2 / 2.0;
    let complete = carlson_rf(vec3f(0.0, p.complement, 1.0));
  ...
  if (p.three_real) {
    ...
    let x = w / scale;
    let y = p.scale / scale;
    sine2 = 2.0 * y / (x + y + sqrt((x - y) * (x - y) + 4.0 * x * y * p.complement));
  }
  if (!(sine2 > 0.0 && sine2 <= 1.0)) { return Arrival(0.0, 0.0, 2u); }
  let complement = (1.0 - sine2) + p.complement * sine2;
  ...
  if (p.complement == 0.0) { return Arrival(0.0, 0.0, 0u); }
  let complete = carlson_rf(vec3f(0.0, p.complement, 1.0));
