import { presets } from "../model/presets.ts";
import { decodeView, encodeView } from "../model/view.ts";
import type { SavedView } from "../model/view.ts";
import { initialSession, transition } from "../model/session.ts";
import type { Action, Session } from "../model/session.ts";
import type { CompletedFrame, RenderEvent } from "../render/protocol.ts";
import { createRenderClient } from "./render-client.ts";
...
export function mountApp(root: HTMLElement, restored?: Session): () => Session {
  ...
  function syncStatus() {
    ...
    photoLabel.textContent =
      samples === 64 ? "Photograph ready" : `Refining light · ${samples} / 64`;
    ...
    if (frame?.coverage) {
      ...
      coverage.textContent =
        value.unresolvedSamples === 0
          ...
          : `${value.unresolvedPixels.toLocaleString("en")} pixels contain unresolved samples (${((100 * value.unresolvedSamples) / (value.pixels * value.samplesPerPixel)).toFixed(4)}% of samples).`;
    } else {
      coverage.textContent =
